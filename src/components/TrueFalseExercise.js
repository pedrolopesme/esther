        ...
        <div className="flex gap-4 my-6 justify-center">
          {options.map((option, index) => (
            <OptionCard 
              ...
              className="flex-1 max-w-[200px] text-center font-bold text-lg py-4"
            >
              {option === 'Verdadeiro' && '✓ Verdadeiro'}
              {option === 'Falso' && '✗ Falso'}
            </OptionCard>
          ))}
        </div>
        ...
        {hasSubmitted && (
          <Feedback correct={isCorrect}>
            {isCorrect 
              ...
              : <>
                  <span className="font-bold">Incorreto. A resposta correta é: {resposta_correta || options[correctIndex]}</span>
                  {explicacao && <p className="mt-2">{explicacao}</p>}
                </>
            }
          </Feedback>
